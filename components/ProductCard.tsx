...
export default function ProductCard({ product }: { product: Product }) {
	...
	return (
		<div className="product-card group">
			...
			<Link href="/products" className="product-card-copy"><p className="eyebrow">{product.category.name}</p><h2>{product.name}</h2><p className="product-description">{product.description}</p><strong>Rp {product.price.toLocaleString('id-ID')}</strong></Link>
